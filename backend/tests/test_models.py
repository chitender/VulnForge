...
def test_registries_has_envelope_columns(db):
    result = db.execute(
        text(
...
def test_merge_requests_has_pipeline_columns(db):
    result = db.execute(
        text(
            "SELECT column_name FROM information_schema.columns "
            "WHERE table_name='merge_requests' AND column_name IN ('gitlab_pipeline_id','pipeline_status','image_digest')"
        )
    )
    cols = {row[0] for row in result.fetchall()}
